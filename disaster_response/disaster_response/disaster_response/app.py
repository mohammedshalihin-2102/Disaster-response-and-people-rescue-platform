...
def team_dashboard():
    conn = get_db()
    assignments = conn.execute("SELECT * FROM assignment").fetchall()
    conn.close()
...
def assignments():
    ...
    assignments = conn.execute("""
        SELECT 
            ...
            assignment.status
        FROM assignment
        JOIN incident ON assignment.incident_id = incident.incident_id
        JOIN rescue_team ON assignment.team_id = rescue_team.team_id
    """).fetchall()

    # Fetch incidents for the Add Assignment form
    incidents = conn.execute("SELECT * FROM incident").fetchall()
    
    # Fetch rescue teams for the Add Assignment form
    teams = conn.execute("SELECT * FROM rescue_team").fetchall()
...
def add_assignment():
    ...
    conn = get_db()
    conn.execute("""
        INSERT INTO assignment (incident_id, team_id, status)
        VALUES (?, ?, ?)
...
def delete_assignment(id):
    conn = get_db()
    conn.execute("DELETE FROM assignment WHERE id = ?", (id,))
    conn.commit()
    conn.close()
